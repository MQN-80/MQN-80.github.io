import{_ as l}from"./plugin-vue_export-helper-c27b6911.js";import{r as p,o as t,c as o,a as n,b as s,e as a,f as i}from"./app-7dd07e0d.js";const c="/assets/go-err-61bce072.jpeg",d="/assets/magic-go-compiler-fa7e7b4e.png",r="/assets/magic-gfs-18c29901.png",u={},m=i(`<h2 id="前言" tabindex="-1"><a class="header-anchor" href="#前言" aria-hidden="true">#</a> 前言</h2><p>几乎所有人都遇到过类似下面的 go 代码：</p><div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token keyword">if</span> err <span class="token operator">!=</span> <span class="token boolean">nil</span> <span class="token punctuation">{</span>
  <span class="token keyword">return</span> <span class="token boolean">nil</span><span class="token punctuation">,</span> err
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>负责任的说，笔者每天都得写很多次这样的 <strong>err</strong> 处理代码，以至于一个函数中，半数代码都是在<br> 处理 error。所以很多人用下面这张图来调侃：</p><p><img src="`+c+'" alt="go-err"></p>',5),v={href:"https://coolshell.cn/articles/21140.html",target:"_blank",rel:"noopener noreferrer"},k={href:"https://www.modb.pro/db/172962",target:"_blank",rel:"noopener noreferrer"},b=i(`<p>一定程度上，这种技巧解决了 go error 处理的繁琐，但显然这种方式还是不够优雅。</p><p>思来想去，根本原因在于 <strong>go</strong> 本身 error 的哲学和机制问题，go 创始人希望工程师能够认真对待每一个 error，从而提升代码质量；<br> 另外 go 语言本身又没有提供错误处理语法糖，这使得 error 处理十分原始和重复。</p><p>既然常规方法不行，笔者就把视角放到了 go 本身上面：给 go 编译器新增<strong>关键字</strong>来处理 error。</p><h2 id="编译源码" tabindex="-1"><a class="header-anchor" href="#编译源码" aria-hidden="true">#</a> 编译源码</h2><p>话不多说，我们直接开搞。</p><p>从 github 上下载 go 的源码，并将其切到（当前）最新的 go1.19 版上：</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token comment"># git clone https://github.com/golang/go  </span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,9),g=n("br",null,null,-1),f=n("br",null,null,-1),h={href:"https://github.com/golang/go/blob/master/src/make.bash",target:"_blank",rel:"noopener noreferrer"},x=i(`<p>这样就通过已有的 go1.18 编译出最新的 go1.19 了：</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token comment"># $pwd/bin/go version</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,26),w={href:"https://github.com/pedrogao/go/commit/7df86e87f555af26cb51dc60484327c0d0eb64b0#diff-e3c67b440212ca6e8ffdb55884bbd573cd4235c439948caeb2d92bebf6f5372c",target:"_blank",rel:"noopener noreferrer"},_=i(`<div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token keyword">func</span> <span class="token function">testErr</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token builtin">error</span> <span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>程序输入 <code>222，err</code>，证明函数 hello 从 r3 处退出，显然 r1，r2 处的 error 都是 nil，因此判断不成立，<br> 所以函数继续执行，而到 r3 处时，error 不为 nil，所以直接返回。</p>`,4),y={href:"https://github.com/pedrogao/go/commit/7df86e87f555af26cb51dc60484327c0d0eb64b0",target:"_blank",rel:"noopener noreferrer"},S=n("h2",{id:"go-编译器",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#go-编译器","aria-hidden":"true"},"#"),s(" go 编译器")],-1),B=n("p",null,[s("到这里，我们可以发现 go 编译器设计良好、代码易读，新增一个可用的关键字竟如此简单。但是旅程绝不仅限于此，"),n("br"),s(" 就目前而言，我们只接触到 go 编译器中极其小的一部分，甚至这些都不是 go 编译器中的核心点。")],-1),q=n("p",null,"go 编译器是按照程序编译过程来设计实现的，大致可分为如下 4 个大阶段：",-1),O=n("p",null,[n("img",{src:d,alt:"compiler"})],-1),D=n("br",null,null,-1),N={href:"https://github.com/golang/go/blob/master/src/cmd/compile/README.md",target:"_blank",rel:"noopener noreferrer"},E=n("br",null,null,-1),I=n("ol",null,[n("li",null,"解析阶段：将 go 源代码词法、句法、语法分析，生成程序可读的语法树（syntax tree）；"),n("li",null,"检查、转换阶段：对生成的语法树做语法检查，补充元信息；"),n("li",null,"生成 SSA 节点，通过流程图分析对代码进行优化、重写；"),n("li",null,"生成机器码，即最后的可执行代码。")],-1),C={href:"https://en.wikipedia.org/wiki/Static_single-assignment_form",target:"_blank",rel:"noopener noreferrer"},T=i(`<p>而 ereturn 语句的处理，只停留在了第 1 阶段，即解析阶段的时候，就被转化为了 if、return 语句，后面的 2、3、4 阶段都是以其它语句的身份来参与。</p><p>下面，我们就通过另外一个关键字 <code>dowhile</code> 来一起看看 go 编译的 4 个阶段。</p><h2 id="dowhile" tabindex="-1"><a class="header-anchor" href="#dowhile" aria-hidden="true">#</a> dowhile</h2><p>有一定 c 语言基础的同学肯定熟悉 <code>do while</code> 这两个关键字，因此它们联合可以实现如下语句：</p><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token keyword">do</span>
<span class="token punctuation">{</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,46),$={href:"https://github.com/pedrogao/go/commit/491af5b8c929fd12b65674a8def061b5cced9da9#diff-103e7d8cdbb3e39378d0cd1b928ca2c9455b4de306c234ab5eeacdc9f3c0ac3c",target:"_blank",rel:"noopener noreferrer"},G=n("br",null,null,-1),P={href:"https://github.com/pedrogao/go/commit/491af5b8c929fd12b65674a8def061b5cced9da9#diff-fe8447c6058eafe833372607bb6d2514d1feb5e3c09b2dddbc8776e58e1acf07",target:"_blank",rel:"noopener noreferrer"},R=n("br",null,null,-1),F=i(`<p>为了方便 DowhileStmt 语句的 debug，因此新增 ODOWHILE 格式化的 case：</p><div class="language-diff line-numbers-mode" data-ext="diff"><pre class="language-diff"><code><span class="token coord">--- a/src/cmd/compile/internal/ir/fmt.go</span>
...
</span></span></code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>CFS 基本已经接近人类理解的程序执行流程图了，上面的代码会将 dowhile 生成如下控制流图：</p><p><img src="`+r+'" alt="cfs"></p><p>dowhile 开始时，直接进入 body 中执行，执行完后再来判断 cond 是否满足，如果满足则再次跳入到 body 中执行，否则 dowhile 结束。</p><p>可以看到，从一个基本的 dowhile 语句到 CFS，编译器做了大量工作，一步步将语言向下演化（这个过程也称 lower）。</p>',17),A={href:"https://github.com/pedrogao/go/blob/pedro/src/cmd/internal/obj/",target:"_blank",rel:"noopener noreferrer"},H=n("br",null,null,-1),L=i(`<p>再次编译 go1.19，此时的 go1.19 已经支持 dowhile 关键字：</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token comment"># cd $pwd/src</span>
<span class="token comment"># ./make.bash</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div>`,2),W={href:"https://github.com/pedrogao/go/commit/491af5b8c929fd12b65674a8def061b5cced9da9#diff-890164fa94bc37c4cd437e241d79d9d1898987a5fb3c6d0a69b0aa9164cee5e3",target:"_blank",rel:"noopener noreferrer"},V=i(`<div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token keyword">func</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    i <span class="token operator">:=</span> <span class="token number">4</span>
    dowhile i <span class="token operator">&gt;</span> <span class="token number">0</span> <span class="token punctuation">{</span>
        fmt<span class="token punctuation">.</span><span class="token function">Println</span><span class="token punctuation">(</span>i<span class="token punctuation">)</span>
        i<span class="token operator">--</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,3),X={href:"https://github.com/pedrogao/go/commit/491af5b8c929fd12b65674a8def061b5cced9da9#diff-efe8aa91b1bad6bf66e574790c88f5b58fd56d87bf47f41ea083368a902b8724",target:"_blank",rel:"noopener noreferrer"},j=i(`<div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token keyword">func</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
...
<span class="token number">2</span>
<span class="token number">1</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,3),z={href:"https://github.com/pedrogao/go/commit/491af5b8c929fd12b65674a8def061b5cced9da9#diff-98d9e0c744509ee4e2f1cd8eb85994a5023c5f0785054abbf7a417b6a1a4c35d",target:"_blank",rel:"noopener noreferrer"},K=i(`<div class="language-go line-numbers-mode" data-ext="go"><pre class="language-go"><code><span class="token keyword">func</span> <span class="token function">main</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
	i <span class="token operator">:=</span> <span class="token number">4</span>
	dowhile i <span class="token operator">!=</span> <span class="token number">4</span> <span class="token punctuation">{</span>
		fmt<span class="token punctuation">.</span><span class="token function">Println</span><span class="token punctuation">(</span>i<span class="token punctuation">)</span>
	<span class="token punctuation">}</span>
...
<span class="token number">4</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>可以发现，即 <code>i != 4</code> 不满足，但仍然执行了一次 <code>fmt.Println</code>。</p>`,4),Y={href:"https://github.com/pedrogao/go/commit/491af5b8c929fd12b65674a8def061b5cced9da9",target:"_blank",rel:"noopener noreferrer"},M=i('<h2 id="结语" tabindex="-1"><a class="header-anchor" href="#结语" aria-hidden="true">#</a> 结语</h2><p>至此，我们完成了 go 编译器新增关键字的挑战，在这一段路的旅程中，我们见识到了编译原理的魅力，感受到了 go 编译器设计之精巧、功能之完善；<br> 无论是编译器本身、还是 runtime 的实现，都是学习（任意）语言实现的巨大宝库。</p><p>按照 go 文档的说法，go 编译器被称为 <code>gc编译器</code>，虽然这一度让我怀疑这是垃圾回收的代码包，但明显这不是。go 自从 1.4 版本自举后，参与编译器工作的<br> 门槛降低了很多，在开发和实践中，我们都能直接去看源码来 debug 程序问题；当然遇到 go 无法解决的问题，我们也能 fork 然后自己来解决。</p><p>c/cpp 编译器一直都是 gcc、clang、MSVC 三家争霸，而 go 编译器似乎一直都是一家独大，这也直接导致了 go 团队的傲慢，当然也避免了分裂和撕逼。</p><p>可能不久的将来，因为或多或少的政治原因，go 编译器也会走向多元化，期待那一天的到来，让 go 拥有更多的活力。</p><h2 id="参考资料" tabindex="-1"><a class="header-anchor" href="#参考资料" aria-hidden="true">#</a> 参考资料</h2>',6),U={href:"https://eli.thegreenplace.net/2019/go-compiler-internals-adding-a-new-statement-to-go-part-2/",target:"_blank",rel:"noopener noreferrer"},J={href:"https://zhuanlan.zhihu.com/p/549728632",target:"_blank",rel:"noopener noreferrer"},Q={href:"https://github.com/pedrogao/go",target:"_blank",rel:"noopener noreferrer"},Z={href:"https://coolshell.cn/articles/21140.html",target:"_blank",rel:"noopener noreferrer"},nn={href:"https://www.modb.pro/db/172962",target:"_blank",rel:"noopener noreferrer"},sn={href:"https://quasilyte.dev/blog/post/go_ssa_rules/",target:"_blank",rel:"noopener noreferrer"};function en(an,ln){const e=p("ExternalLinkIcon");return t(),o("div",null,[m,n("p",null,[s("遇到如此重复错误处理代码，大家都会下意识的想办法去封装和简化，比如这里的两篇文章"),n("a",v,[s("GO 编程模式：错误处理"),a(e)]),s(" 和 "),n("a",k,[s("Go error 处理最佳实践"),a(e)]),s("。")]),b,n("blockquote",null,[n("p",null,[s("从构建输出可以看出，go 的自举其实颇为复杂。首先使用旧 go1.18.3 编译构建 toolchain1，"),g,s(" 然后再用 toolchain1 构建出 bootstrap，最后几次迭代编译构建出 go1.19 和其它工具，"),f,s(" 感兴趣的可以 "),n("a",h,[s("点击"),a(e)]),s(" 看看 go 的构建过程。")])]),x,n("p",null,[s("编译完成后，用一个"),n("a",w,[s("例子"),a(e)]),s("看看效果：")]),_,n("p",null,[s("到此一个新的 ereturn 关键字就顺利完成了，具体实现可以参考这个 "),n("a",y,[s("commit"),a(e)]),s("。")]),S,B,q,O,n("blockquote",null,[n("p",null,[s("以上图片按照 go 官方文档介绍绘制而成，感兴趣的可以点击"),D,n("a",N,[s("这里"),a(e)]),s("查看一下 go 编译器的"),E,s(" 简单介绍。文档可能会随着迭代更新。")])]),I,n("blockquote",null,[n("p",null,[s("关于 SSA，可点击"),n("a",C,[s("此处"),a(e)]),s("了解。")])]),T,n("p",null,[s("generate 命令会重新生成"),n("a",$,[s("op_string.go"),a(e)]),s("文件，"),G,s(" 用于将 ODOWHILE 关键字转化为字符串方便 debug。")]),n("p",null,[s("run 命令会重新生成"),n("a",P,[s("node_gen.go"),a(e)]),s("文件，"),R,s(" 用于自动实现 DowhileStmt 语句的格式化、拷贝等操作。")]),F,n("p",null,[s("最后，就是编译器的最后一步了，将 CFS 图转化为对应平台的机器码，对应 "),n("a",A,[s("obj"),a(e)]),s("包；"),H,s(" 包下有对各个平台的机器码支持，由于平台之间的差异都比较大，因此我们对 dowhile 的支持其实到 IR 这一步就够了，剩下的工作 go 编译器会帮助我们来处理。")]),L,n("p",null,[s("我们可以测试看看，一个 "),n("a",W,[s("dowhile 例子"),a(e)]),s("：")]),V,n("p",null,[s("例子 2："),n("a",X,[s("dowhile 语句上初始化"),a(e)]),s("：")]),j,n("p",null,[s("例子 3："),n("a",z,[s("dowhile 条件不满足"),a(e)]),s("：")]),K,n("p",null,[s("关于 dowhile 的具体实现可以参考这个"),n("a",Y,[s("commit"),a(e)]),s("。")]),M,n("ul",null,[n("li",null,[n("a",U,[s("go compiler internals"),a(e)])]),n("li",null,[n("a",J,[s("静态分析-中间表示"),a(e)])]),n("li",null,[n("a",Q,[s("golang"),a(e)])]),n("li",null,[n("a",Z,[s("GO 编程模式：错误处理"),a(e)])]),n("li",null,[n("a",nn,[s("Go error 处理最佳实践"),a(e)])]),n("li",null,[n("a",sn,[s("Go compiler: SSA optimization rules description language"),a(e)])])])])}const on=l(u,[["render",en],["__file","magic3.html.vue"]]);export{on as default};
